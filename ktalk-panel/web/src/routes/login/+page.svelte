...
<svelte:head><title>Login — Admin Panel</title></svelte:head>

<div class="min-h-screen flex items-center justify-center bg-gray-950">
	<div class="w-full max-w-sm bg-gray-900 rounded-2xl p-8 shadow-xl">
		<h1 class="text-2xl font-bold text-white mb-6 text-center">⚡ Admin Panel</h1>
		{#if error}
			<div class="mb-4 p-3 bg-red-900/40 border border-red-700 rounded-lg text-sm text-red-300">{error}</div>
		{/if}
		<form on:submit|preventDefault={login} class="space-y-4">
			<div>
				<label for="password" class="text-xs text-gray-400 block mb-1">Admin password</label>
				<input
					id="password"
					type="password"
					bind:value={password}
